---
title: "Comparison of different normalization methods for bigwigs"
...
The data is ACT-Seq of H3K27ac, binned in 10kb for computing time reasons. 

The data was either normalizes with 

* BPM
* CPM
* per spike in with the cwlab workflow
* SPM with MACS2
* RPGC to an effective genome size of 2.7*10^9 bp 
* RPKM
* Not at all 
...
norm <- list.files("/omics/groups/OE0219/internal/ganslm/Analysis/Mouse_ActSeq/bw_comparison")

paths <- list.files("/omics/groups/OE0219/internal/ganslm/Analysis/Mouse_ActSeq/bw_comparison",
           pattern = glob2rx("*mbw_summary_10kb_*.mat"),
                         recursive = T, 
                         full.names = TRUE)
datas <- list()
...
for(i in 1:length(paths)){
  datas[[i]] <- read.table(paths[i], stringsAsFactors=FALSE, header=TRUE, comment.char="!"
                           )
  NN <- norm[i]

  colnames(datas[[i]]) <- c(colnames(datas[[i]][,1:3]), paste0(NN, "_", colnames(datas[[i]][, -c(1:3)]))) 

  }


data <- purrr::reduce(datas, dplyr::inner_join, by = c("X..chr.", "start", "end"))
...
col_annot <- data.frame(H3F3A=c("WT","MT")[1+(sampls_mark%in% mt_samples)], 
                        Sample=sapply(strsplit(split="_",colnames(top_mat)), "[", 6), 
                        Normalization=sapply(strsplit(split="_",colnames(top_mat)), "[", 1))

colx <- brewer.pal(n = length(norm), name = "Accent")
names(colx) <- norm
...
full_data$Sample<-sapply(strsplit(full_data$Stat, split="_"),"[",6)
full_data$Mark<-sapply(strsplit(full_data$Stat, split="_"),"[",7)
full_data$Normalization<-sapply(strsplit(full_data$Stat, split="_"),"[",1)
...
sum_full_data <- full_data %>% 
    group_by(Group,Mark,Region,Feature, Sample, Normalization)%>% 
...
sum_full_data <- bind_rows(listed)

saveRDS(sum_full_data, "output/sum_full_data.Rds")
```


```{r results='asis', eval=TRUE}
sum_full_data <- readRDS("output/sum_full_data.Rds")
norm <- list.files("/omics/groups/OE0219/internal/ganslm/Analysis/Mouse_ActSeq/bw_comparison")
...
plot_seperate <- sums %>%
  ...
  theme(axis.text.y = element_blank(),
        ...
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank())+
  facet_wrap(Sample~replicate, ncol = 1)+
  theme(strip.background=element_rect(fill="white"))
