...
#for(element in norm){
element <- "RPKM"  
file <- paste0("/omics/groups/OE0219/internal/ganslm/Analysis/Mouse_ActSeq/bw_comparison/", element, 
               "/mbw_summary_10kb_", element, ".mat") 


#read in matrix from multiBigWigSummary
  
data <-read.table(file, stringsAsFactors=FALSE, header=TRUE, comment.char="!")
...
plot_seperate <- sums %>%
  ...
  theme(axis.text.y = element_blank(),
        ...
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank())+
  facet_grid(Sample~replicate, drop = T, cols = 1)+
  theme(strip.background=element_rect(fill="white"))
